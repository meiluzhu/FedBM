...
OCT_Concepts = ['Age-related macular degeneration', 'Choroidal neovascularization', 'Central serous retinopathy', 'Diabetic macular edema', 'Macular hole', 'Drusen', 'Diabetic retinopathy', 'Normal']
...
class Data(object):
    def __init__(self, args):
        ...
        if args.dataset == 'OCT':
            ...
            self.train_set = OCT(args.data_path, 'train', tra_transformer, in_memory=False)
            ...
            self.val_set = OCT(args.data_path, 'val', val_transformer, in_memory=False)
            ...
            self.test_set = OCT(args.data_path, 'test', val_transformer, in_memory=False)
            ...
            self.train_set = Kvasir(args.data_path, 'train', tra_transformer, in_memory=False)
            if args.iid == 0:  # noniid
                random_state = np.random.RandomState(int(args.random_seed))
                num_indices = len(self.train_set)
                groups, proportion = build_non_iid_by_dirichlet(random_state=random_state, dataset=self.train_set, non_iid_alpha=args.dirichlet_alpha, num_classes=num_classes, num_indices=num_indices, n_workers=node_num)  
            ...
            self.val_set = Kvasir(args.data_path, 'val', val_transformer, in_memory=False)
            self.val_loader=DataLoader(self.val_set, batch_size=self.args.batchsize, num_workers=2, shuffle=False)
            print('Val sample size', len(self.val_set))
            
            self.test_set = Kvasir(args.data_path, 'test', val_transformer, in_memory=False)
            self.test_loader=DataLoader(self.test_set, batch_size=self.args.batchsize, num_workers=2, shuffle=False)
            print('Test sample size', len(self.test_set))
        else:
            assert False
